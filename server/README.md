...
跟手机扫码上传的分工：扫码管「人在店里、局域网、秒级插播」，这里管「人不在店里、走公网、改正式内容」。
扫码传的东西优先级最高、不受这边影响。
...
- 页面上不会出现「保存」中间态：**发布就是写清单**，写之前素材必须都已在线上，
  否则整个发布中止并点名是哪个素材没传完。
- 清单里的 `size` **由服务端按线上文件的实际大小填**，不采信页面报的数（页面可能是错的）。
- 网页和命令行**共用同一份清单生成逻辑**（`tools/signage_core.py`）。两边各写一套的话，
  改一处忘一处，而端侧对清单不一致的表现只是「某些素材不更新」，极难排查。
...
**时间一律以服务器为准**：老电视的系统时钟经常是错的（差几年的都见过），页面上的「多久没心跳」
按服务器收到的时刻算，端侧报上来的时间只作参考。

**回传坏了不影响播放**：地址填错也只是自认失败，屏照播。连续失败 3 次后改成每 3 次机会试一次，
不会一直白敲。
